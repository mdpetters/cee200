...
function state_space_solver(A, B, C, D, u0, f, tspan)
	...
	solution = solve(problem, BS3(), reltol = 1e-6, abstol = 1e-6)
...
outvec = let
	...
	f(t) = [sqrt(myV[1]), 280.0, 1.0, 300.0] # V = 5V, Th = 280K, 1, Tenv = 300K
...
# ╔═╡ f391211f-7086-4865-894b-7f29018cda7c
...
outvec1 = let
	...
	u0 = [300.0]
	tspan  = [0.0, 3000.0]                   # 0 to 1000 s
	f(t) = [(sin(0.01*t) + 1.0)/2.0*sqrt(3) + 1, 280.0, 1.0, 300.0]    
...
begin
	...
	p1 = plot(0:1:3000, fex.(collect(0:1.0:3000)).^2, color = :black, label = :none,
		ylabel = "V")
...
The `update_system` function incrementally solves the differential equation in `dt` intervals, where `dt` is the time step. It can be thought of a virtual observer which reports the system output after a `dt` interval.
"""

# ╔═╡ 54a7e106-4537-4857-a9e7-fdc3e9bf4ba1
function update_system(T, V, dt)
	m = 0.3                          # kg
	c = 502.0                        # J kg-1 K-1
	k = 200.0*(0.025*0.025)/(m*c)    # s^-1
	a1 =  11.07 				     # W V^-0.5 
	...
	f(t) = [sig.*sqrt(abs(V)), 280.0, 1.0, 300.0]    
...
# Example simulation
ts, Ts, Vs = simulation(300.0, 5.0, 3600)

# ╔═╡ 43c38357-4d59-457d-9b90-94ae1f267212
plot(ts./60, Ts, color = :black, label = :none)

# ╔═╡ f85d4b96-e5d8-4d5c-aaed-df30c8c06084
function simulation_onoff(T, n; Tset = 280.0)
	dt = 1.0 
	ts = Float64[]
	Ts = Float64[]
	Vs = Float64[]
...
let 
	Tset = 260.0
	ts, Ts, Vs = simulation_onoff(270.0, 180; Tset = Tset)
	p1 = plot(ts./60.0, Vs, label = :none, color = :black, ylabel = "V")
	p2 = plot(ts./60.0, Ts, color = :black, ylabel = "T (K)")
	plot!([0, ts[end]./60.0], [Tset, Tset])
	plot(p1, p2, layout = grid(2,1))
end

# ╔═╡ b40aefaa-4600-42f7-ab1d-d92b54fb6401
function simulation_P(T, n; Tset = 280.0)
	span = 20.0
	Kp = 1.4
	dt = 1.0 
	ts = Float64[]
...
let 
	Tset = 290.0
	ts, Ts, Vs = simulation_P(300.0, 3600; Tset = Tset)
	p1 = plot(ts./60.0, Vs, label = :none, color = :black, ylabel = "V")
	p2 = plot(ts./60.0, Ts, color = :black, ylabel = "T (K)")
	plot!([0, ts[end]./60.0], [Tset, Tset])
	plot(p1, p2, layout = grid(2,1))
...
function simulation_PI(T, n; Tset = 280.0)
	span = 20.0
	Kp = 1.4
	dt = 1.0 
	ts = Float64[]
